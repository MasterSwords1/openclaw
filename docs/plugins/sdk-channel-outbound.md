...
A transport whose native API accepts idempotency keys can make an ambiguous queued send replay-safe. After any preparatory provider work such as a media upload, persist the exact recipient-visible finalization plan before its first idempotent native call. Key the plan by `deliveryQueueStateDir`, `deliveryQueueId`, `deliveryPayloadIndex`, and `deliveryPartIndex`; the state directory keeps identical queue IDs from different queue roots distinct. For fan-out sends, also persist the supplied `deliveryPayloadCount` and exact ordered `deliveryPartIndexes` before the first native call. Recovery must validate those authoritative coordinates rather than infer them from current chunking or media settings. Invoke `onPlatformSendDispatch` only after that plan is durable and immediately before the first recipient-visible finalization call. Recovery receives the same queue coordinates and must reuse the stored plan without rerunning modifying hooks or rebuilding media requests.

When `reconcileUnknownSend(...)` verifies that the stored plan still matches the active account, destination, and provider idempotency scope, it may return `{ status: "replay_safe" }`. Core automatically enables unknown-send reconciliation for an ordinary queued send when the adapter declares support for its final prepared payload shape, unless the caller explicitly sets `requireUnknownSendReconciliation: false`. Return `unresolved` and fail closed if the plan is absent, corrupt, or no longer matches. Use `afterQueueTerminal(...)` to delete provider-owned plans only after core has committed a non-replayable queue terminal. Cleanup is best-effort; pair it with `api.runtime.state.getOutboundDeliveryQueueStatus(...)` garbage collection so an interrupted deletion cannot retain plans indefinitely.
