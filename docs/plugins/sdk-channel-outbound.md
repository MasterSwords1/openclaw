...
A transport whose native API accepts idempotency keys can make an ambiguous queued send replay-safe. After any preparatory provider work such as a media upload, persist the exact recipient-visible finalization plan before its first idempotent native call. Key the plan by `deliveryQueueStateDir`, `deliveryQueueId`, `deliveryPayloadIndex`, and `deliveryPartIndex`; the state directory keeps identical queue IDs from different queue roots distinct. Invoke `onPlatformSendDispatch` only after that plan is durable and immediately before the first recipient-visible finalization call. Recovery receives the same queue coordinates and must reuse the stored plan without rerunning modifying hooks or rebuilding media requests.
