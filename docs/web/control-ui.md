...
<AccordionGroup>
  ...
  <Accordion title="Automations panel notes">
    ...
    - Set `cron.webhookToken` and approve each exact HTTPS receiver in `cron.webhookTokenDestinations` to send a dedicated bearer token. Unapproved receivers get no auth header.
