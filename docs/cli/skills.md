...
| `install/update --acknowledge-install-policy-warning` | Continues a reviewed noninteractive request after `security.installPolicy` returns `warn`. OpenClaw reruns policy; this cannot override `block`.                                                                                                                                  |
...
`--acknowledge-install-policy-warning`; OpenClaw reruns the policy, and a
