          ...
          ? {
              disposeRuntime: async () => {
                await retireSessionMcpRuntimeInstance({
                  ...
                  // A newer overlapping turn keeps its own lease, so stale
                  // cleanup leaves the shared instance intact.
                  preserveActiveLeases: true,
                  onError: (error, sessionId) => {
                    log.warn(
                      `bundle-mcp cleanup failed after run for ${sessionId}: ${String(error)}`,
                    );
                  },
                });
              },
