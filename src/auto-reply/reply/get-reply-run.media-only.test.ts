...
describe("runPreparedReply media-only handling", () => {
  ...
  it("restores originalPath to path and originalUrl to url when persisting user turn media", async () => {
    const params = baseParams();
    params.ctx.ThreadHistoryBody = undefined;
    params.ctx.media = [
      {
    ...
    const fact = (persisted as { __openclaw?: { media?: unknown[] } })?.["__openclaw"]
      ?.media?.[0] as Record<string, unknown> | undefined;
    expect(fact).not.toHaveProperty("originalPath");
    expect(fact).not.toHaveProperty("originalUrl");
