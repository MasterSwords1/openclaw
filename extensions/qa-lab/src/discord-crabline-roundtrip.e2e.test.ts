...
describe("Discord Crabline real-plugin roundtrip", () => {
  it.runIf(RUN_DISCORD_CRABLINE_E2E)(
    ...
    async () => {
      ...
      // Discord HTTP server and recorder are closed. The private descriptor lives in the removed
      // child temp root and must never escape into durable suite artifacts.
      await expect(
        fs.access(path.join(suite.result.outputDir, "discord-endpoint-runtime.json")),
