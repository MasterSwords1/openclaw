  ...
  async getTransactionScopeId(): Promise<string> {
    ...
    const active =
      ...
      (async () => {
        ...
        // Matrix v1.7 makes transaction identifiers device-scoped. Older homeservers
        // remain token-scoped, so changing tokens must not reuse a persisted plan.
        const deviceScoped =
          deviceId !== null &&
          supportsDeviceScopedTransactions((await this.client.getVersions()).versions);
        const scopeKind = deviceScoped ? "device" : "access-token";
        const scopeId = deviceScoped ? deviceId : this.transactionScopeAccessTokenHash;
